...
import { useUpdateDeckboxCard, useDeleteDeckboxCard } from '@/hooks/useDeckboxCards'
...
type Props = {
  deckboxId: string
  card: DeckboxCard | null
  onClose: () => void
}

export function DeckboxCardDetailDialog({ deckboxId, card, onClose }: Props) {
  const update = useUpdateDeckboxCard(deckboxId)
  const del = useDeleteDeckboxCard(deckboxId)
  ...
  function onDelete() {
    if (!card) return
    if (!confirm('Retirer cette carte de la deckbox ?')) return
      ...
      >
        ...
        <form onSubmit={onSubmit} className="space-y-4">
          ...
          <div className="flex items-center justify-between pt-2">
            ...
            >
              {del.isPending ? 'Retrait…' : 'Retirer de la deckbox'}
            </button>
